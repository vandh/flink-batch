...
public class LoadData2Mysql4 {
    ...
    public LoadData2Mysql4(String[] args) {
       /* if(args.length != 8) {
        ...
        this.ip = args[1+8];
        this.port = args[2+8];
        this.dbname = args[3+8];
        this.username = args[4+8];
        this.password = args[5+8];
        this.filepath = args[6+8];
        this.table = args[7+8].toUpperCase();
//        this.batch = args[8];
        ...
        try {
            ...
            for(int i=0; i<files.length; i++) {
                ...
                if (pathDirName.indexOf("PRE")!=-1)
                    //批次文件插入到对应 批次的表
                   table="ADD_AP_PRE_VOUCHER_INFO_"+ pathDirName.substring(pathDirName.lastIndexOf(1));
                    //执行批次文件插入到同一张表
