...
public class APExecute_STD4 implements BaseExecute {
    ...
    public DataSet<Row> run(ExecutionEnvironment env) throws RuntimeException {
        ...
        CsvReader csvReader = env.readCsvFile(Constants.OUTPATH + File.separator + Constants.APSTD25 + "-" + Constants.batch);
        input1 = csvReader.includeFields("111111111111111111111")
                .lineDelimiter("\n")
                .fieldDelimiter(",")
                .ignoreInvalidLines()
        ...
        PO_VENDOR_SITES_ALL.proc(env).join(input1)
                .where(5)
                .equalTo(0)
