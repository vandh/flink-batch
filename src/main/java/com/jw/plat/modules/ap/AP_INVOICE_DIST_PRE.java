...
public class AP_INVOICE_DIST_PRE {
    /**
     * 抽取的列数据：
     * ACCOUNTING_DATE      1
     * INVOICE_ID   8
    ...
    public static DataSet<Tuple3<String,String,String>> proc(ExecutionEnvironment env) {
        CsvReader csvReader = env.readCsvFile(FileUtil.getFileName(Constants.PATH, Constants.AP_INVOICE_DIST_PRE, Constants.batch));
        csvReader.setCharset(Constants.GLCHARSET);
        DataSource<Tuple3<String,String,String>> ds1 = csvReader.includeFields(COLMASK)
                        .lineDelimiter(Constants.LF)
                        .fieldDelimiter(Constants.DEL)
                        .ignoreInvalidLines()
                        .types(String.class,String.class,String.class);
        return ds1;
//                ds1.filter(new FilterFunction<Tuple3<String, String, String>>() {
//            @Override
//            public boolean filter(Tuple3<String, String, String> t3) throws Exception {
//                if(Constants.GLDATE==null || Constants.GLDATE.trim().equals("")) return true;
//                String zq = t3.f0.trim();
//                return Constants.GLDATE.indexOf(zq) != -1;
//            }
//        });
    }
